...
/// \brief helps ensure that wakeups of wait()ing
/// parents are not lost. helps obey the
/// memory model when using p->parent.
/// must be acquired before any p->lock.
struct spinlock wait_lock;

/// \brief Allocate a page for each process's kernel stack.
...
/// \brief a user program that calls exec("/init")
/// od -t xC initcode
...
/// \brief Switch to scheduler.  Must hold only p->lock
...
/// \brief A fork child's very first scheduling by scheduler()
...
/// \brief Wake up all processes sleeping on chan.
...
/// \brief Copy from either a user address, or kernel address,
